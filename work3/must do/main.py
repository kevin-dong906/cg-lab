...
@ti.func
def reflect_vector(incoming, normal):
...
def draw_scene():
    for x, y in screen:
        ...
        if closest_dist < 1e9:
            ...
            refl_dir = unit_vector(reflect_vector(-light_dir, norm))
            spec_factor = ti.max(0.0, refl_dir.dot(view_dir)) ** shininess_exp[None]
...
def launch():
    app = ti.ui.Window("Custom Phong Renderer", (WIDTH, HEIGHT))
